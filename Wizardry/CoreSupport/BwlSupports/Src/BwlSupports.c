...
{
    return NBwl_GetBwlData(characterId)->supports;
}
...
{
    int const count = GetUnitSupporterCount(unit);
    ...
    for (int i = 0; i < count; ++i)
    {
        struct Unit* const other = GetUnitSupporterUnit(unit, i);

        if (!other)
            continue;

        GetUnitSuportData(other)[GetUnitSupporterNum(other, charId)] = 0;
    }
}
...
void NuUnitLoadSupports(struct Unit* unit)
{
    ...
    for (int i = 0; i < count; ++i)
        data[i] = GetUnitSupporterInitialExp(unit, i);
}
